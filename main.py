...
if __name__ == "__main__":
    ...
    logger.info(
        "Local DingTalk data sync: "
        + ("enabled" if config.SYNC_LOCAL_DINGTALK_DATA else "disabled")
    )
    if config.SYNC_LOCAL_DINGTALK_DATA:
        logger.info(f"Sync interval: every {config.SYNC_INTERVAL_HOURS} hours")

    # Startup validation: warn early about missing components
    if config.SYNC_LOCAL_DINGTALK_DATA and not os.path.isfile(config.DINGWAVE_PATH):
        logger.warning(
    ...
    if config.SYNC_LOCAL_DINGTALK_DATA and not os.path.exists(config.ENCRYPTED_DB):
        logger.warning(
